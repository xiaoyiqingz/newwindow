...
//加载图片
bool CMyImageEx::LoadImage( HINSTANCE hInstance, LPCTSTR pszResourceName, LPCTSTR pszType)
{
	...
	{
		//查找资源
		hResource=FindResource(hInstance,pszResourceName,pszType);
...
//加载图片
bool CMyImageEx::LoadImage( void * pDate ,DWORD dwSize )
{
	if (IsNull()==false) return false;
...
//创建区域
bool CMyImageEx::CreateImageRegion(CRgn & ImageRgn, COLORREF crTransColor)
{
	//创建区域
...
//创建区域
bool CMyImageEx::CreateImageRegion(CRgn & ImageRgn, INT nXSrcPos, INT nYSrcPos, INT nSrcWidth, INT nSrcHeight, COLORREF crTransColor)
{
	//判断状态
	ASSERT(IsNull()==false);
...
//平铺绘画
bool CMyImageEx::DrawImageTile(CDC * pDC, CRect & rcDestRect)
{
	//平铺绘画
...
//平铺绘画
bool CMyImageEx::DrawImageTile(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
	//效验状态
	ASSERT(IsNull()==false);
	if (IsNull()==true) return false;
...
//一般绘画
BOOL CMyImageEx::BitBlt( HDC hDestDC, int xDest, int yDest, DWORD dwROP /*= SRCCOPY*/ )
{
	return BitBlt(hDestDC, xDest, yDest, GetWidth(), GetHeight(), 0, 0);
}

//一般绘画
BOOL CMyImageEx::BitBlt( HDC hDestDC, const POINT& pointDest, DWORD dwROP /*= SRCCOPY*/ )
{
	return BitBlt(hDestDC, pointDest.x, pointDest.y, GetWidth(), GetHeight(), 0, 0);
}

//一般绘画
BOOL CMyImageEx::BitBlt( HDC hDestDC, const RECT& rectDest, const POINT& pointSrc, DWORD dwROP /*= SRCCOPY*/ )
{
...
BOOL CMyImageEx::BitBlt( HDC hDestDC, int xDest, int yDest, int nDestWidth, int nDestHeight, int xSrc, int ySrc, DWORD dwROP /*= SRCCOPY */ )
{
	return CImage::BitBlt(hDestDC, xDest, yDest, nDestWidth, nDestHeight, xSrc, ySrc, dwROP);
}

//透明绘画
bool CMyImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, COLORREF crTransColor)
{
	//绘画图片
...
//透明绘画
bool CMyImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, COLORREF crTransColor)
{
	//无效区域
	CRect rcDirty;
...
//混合绘画
bool CMyImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, COLORREF crTransColor)
{
	//创建缓冲
	if ((cxDest!=cxSrc)||(cyDest!=cySrc))
	{
		//创建位图
		CMyImageEx ImageStretch;
...
//混合绘画
bool CMyImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, BYTE cbAlphaDepth)
{
	//透明绘画
...
//混合绘画
bool CMyImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, BYTE cbAlphaDepth)
{
	//无效区域
	CRect rcDirty;
	pDestDC->GetClipBox(&rcDirty);
...
//混合绘画
bool CMyImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, BYTE cbAlphaDepth)
{
	//创建缓冲
	if ((cxDest!=cxSrc)||(cyDest!=cySrc))
	{
...
//混合绘画
bool CMyImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, COLORREF crTransColor, BYTE cbAlphaDepth)
{
	//透明绘画
...
//混合绘画
bool CMyImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, COLORREF crTransColor, BYTE cbAlphaDepth)
{
	//无效区域
	CRect rcDirty;
	pDestDC->GetClipBox(&rcDirty);
...
//混合绘画
bool CMyImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, COLORREF crTransColor, BYTE cbAlphaDepth)
{
	//创建缓冲
	if ((cxDest!=cxSrc)||(cyDest!=cySrc))
	{
...
//矩形检查
bool CMyImageEx::IsRectIntersect(const CRect & rcRect, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
	//范围检测
...
//区域计算
bool CMyImageEx::GetDrawImageArea(INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, CRect & rcDirty, tagImageRender & ImageRender)
{
	//相交矩形
	CRect rcIntersect;
