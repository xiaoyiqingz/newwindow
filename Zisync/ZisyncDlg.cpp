// ZisyncDlg.cpp : implementation file
//
...
CZisyncDlg::CZisyncDlg(CWnd* pParent /*=NULL*/)
	: CZiDialog(CZisyncDlg::IDD, pParent)
{
	m_bIsInit = false;
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CZisyncDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CZisyncDlg, CZiDialog)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_ERASEBKGND()
	ON_NOTIFY(NM_CLICK, IDC_LIST1, &CZisyncDlg::OnClickList1)
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB1, &CZisyncDlg::OnSelchangeTab1)
	ON_BN_CLICKED(IDC_SET, &CZisyncDlg::OnBnClickedSet)
	ON_WM_SIZE()
	ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()


// CZisyncDlg message handlers

BOOL CZisyncDlg::OnInitDialog()
{
...
HCURSOR CZisyncDlg::OnQueryDragIcon()
{
...
BOOL CZisyncDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
...
void CZisyncDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CZisyncDlg::SetControlPos(int cx, int cy)
{
	//变量定义
...
void CZisyncDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	PostMessage(WM_NCLBUTTONDOWN, HTCAPTION, MAKELPARAM(point.x, point.y));
	CZiDialog::OnLButtonDown(nFlags, point);
}

BOOL CZisyncDlg::OnEraseBkgnd(CDC* pDC)
{
/*
	CRect rcClient;
	GetClientRect(&rcClient);
...
void CZisyncDlg::DrawClientArea(CDC*pDC,int nWidth,int nHeight)
{
	CRect rcClient;
	GetClientRect(&rcClient);
...
void CZisyncDlg::OnClickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CZisyncDlg::OnSelchangeTab1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CZisyncDlg::OnBnClickedSet()
{
	CDialogSet dlg;
	dlg.DoModal();
}
